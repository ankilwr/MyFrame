...
public class OverScrollView extends ScrollView {
	 ...
	 * 滚动最大高度,超过此高度不再滚动
	 */
	private int OVERSCROLL_MAX_HEIGHT = 5000;
	/**
	 * 滑动的方向 true朝下滑动，为false朝上滑动
	 */
	private boolean UpOrBottom = true;
	/**
	 * 是否可以向下滑动,true可以向下滑动，false不可以向下滑动
	 */
	private boolean ISBOTTOM = true;
	/**
	 * 是否可以向上滑动，true可以向上滑动，false不可以向上滑动
	 */
	...
	/**
	 * 主要是用在用户手指离开ScrollView，ScrollView还在继续滑动，我们用来保存Y的距离，然后做比较
	 */
	...
	public boolean onTouchEvent(MotionEvent ev) {
		isOnTouch = true;
		/*
		 * 当用户的手在ScrollView上面的时候，
		 * 直接将ScrollView滑动的Y方向距离回调给onScroll方法中，当用户抬起手的时候，
		 * ScrollView可能还在滑动，所以当用户抬起手我们隔15毫秒给handler发送消息，在handler处理
		 */
		handler.sendMessageDelayed(handler.obtainMessage(), 15);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (isRecord) {
				...
				// 判断是否朝上滑动（deltaY>0为true朝下滑动，为false朝上滑动）
				UpOrBottom = deltaY > 0 ? ISUP : ISBOTTOM;
				// 记录新的触摸位置
				mLastMotionY = y;
	...
	/**
	 * 功能说明：设置ScrollView滚动距离的监听
	 * @author 作者：jarvisT
	 * @date 2015-1-30 下午3:46:01 
	 * @param listener
	 */
	public void setOnScrollDistanceListener(OnScrollDistanceListener listener){
		mOnScrollDistanceListener=listener;
	}
	...
	/***
	 * 功能说明：overScroll滚动的回调
	 * 
	 * @author 作者：jarvisT
	 * @date 2015-1-30 下午3:36:07
	 */
	...
	/**
	 * 功能说明：是否可以向上滚动 作者：ts 创建日期:2014-11-7 参数：
	 * 
	 * @param isUp
	 *            true可以向上滚动，false不可以
	 */
	...
	/**
	 * 功能说明：是否可以向下滚动 作者：ts 创建日期:2014-11-7 参数：
	 * 
	 * @param isDown
	 *            true可以向下滚动，false不可以
	 */
	public void setIsDown(boolean isDown) {
		this.ISBOTTOM = isDown;
	}

	/**
	 * 功能说明：能够滚动的最大距离 作者：ts 创建日期:2014-11-7
	 */
	public void setScrollMaxHight(int scrollMaxHight) {
		this.OVERSCROLL_MAX_HEIGHT = scrollMaxHight;
	}
}
